...
- [XIOS](https://gitlab.in2p3.fr/ipsl/projets/xios-projects/xios) : I/O dataflow library in which simulation codes clients can send or receive data to a remote server that performs online operation (writing, reading, coupling, post-processing...), including user-defined Python procedures (under development).
- [PhyDLL](https://gitlab.com/cerfacs/phydll) : Coupling library with Fortran/C/Python APIs. Allows coupled scripts (heterogeneously written or not) to exchange data. Built on [CWIPI](https://w3.onera.fr/cwipi/fr).
